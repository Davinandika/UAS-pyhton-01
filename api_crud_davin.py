...
def write_db_api():
	...
	data = response.json()
	cursor = mydb.cursor()
	sql = '''INSERT INTO tbl_students_0493(id,nim,nama,jk,jurusan,alamat) VALUES (%s, %s, %s, %s, %s, %s )'''
	for i in data['data'] :
...
def tampil_data():
	...
	mycursor.execute("SELECT * FROM tbl_students_0493")
...
def tampil_limit():
	baris = int(input("\ntampilkan limit : "))
	mycursor = mydb.cursor()
	sql = ("SELECT * FROM tbl_students_0493")
...
def tampilkan_nim():
	...
	mycursor = mydb.cursor()
	sql = ("SELECT * FROM tbl_students_0493 WHERE nim= ")
